...
// State management - REMOVED currentAppointmentId
let oAuth2Client: any = null;
let calendar: any = null;
const calendarId: string = "primary";
...
const setAppointment = async (
  ...
  try {
    ...
    const bookingData = {
      ...
      timeZone: event.start.timeZone || "UTC",
      googleEventId: response.data.id || null, // Each booking gets its unique event ID
      meetLink: response.data.hangoutLink || null,
      calendarLink: response.data.htmlLink || null,
      status: response.data.status ? "confirmed" : "unknown",
    };
    ...
    // Return the event data (no longer storing in global state)
    return response.data as CalendarEvent;
  } catch (error) {
    console.error("Error setting appointment:", (error as Error).message);
    if ((error as any).response) {
...
// // Updated to accept eventId as parameter
// const getAppointment = async (
//   eventId: string
// ): Promise<CalendarEvent | { message: string }> => {
//   try {
//     if (!oAuth2Client || !calendar) initializeOAuthClient();
